<?php 
return [
    'header' => [
        'title' => 'Exova Indonesia',
        'description' => 'Exova Indonesia adalah platform yang berperan sebagai jembatan penghubung antara penjual dan pembeli jasa dokumentasi secara cepat, mudah, dan fleksibel',
        'search' => 'Apa yang ingin kamu cari?',
        'meta' => [
            'description' => 'Exova Indonesia adalah platform yang berperan sebagai jembatan penghubung antara penjual dan pembeli jasa dokumentasi secara cepat, mudah, dan fleksibel',
            'keywords' => 'Freelancer, Exova Indonesia, Exova, E-Commerce, Freelancer Platform, Lowongan Pekerjaan, Marketplace, Studio Online, Desainer, Photographer, Videographer, Web Developers, Apps Developers, Undangan Online, Online Invitations, Company Profile, Portfolio, Dokuentasi, Tradisi, budaya',
        ]
    ],
    'wallet' => [
        'balance' => 'Saldo',
        'withdraw' => 'Tarik',
        'send' => 'Transfer',
        'mywallet' => 'Wallet Saya',
        'history' => 'Riwayat',
        'code' => 'Kode ExoWallet',
    ],
    'services' => [
        'title' => 'Layanan Kami',
        'jasatitle' => 'Exova Project',
        'createtitle' => 'Exove Creations',
        'jasa' => [
            'description' => 'Exova Project adalah tempat mencari/menjual jasa seperti photography, design, dan videography yang berfokus pada "Capture Moment"',
            'button' => 'Lihat Selengkapnya',
        ],
        'creations' => [
            'description' => 'Exova Creations adalah layanan untuk membuat sesuatu seperti undangan online, web portofolio, hingga web company profile dengan cepat',
            'button' => 'Lihat Selengkapnya',
        ],
    ],
    'highlight' => [
        'seeall' => 'Lihat Semua',
    ],
    'membership' => [
        'subscribe' => 'Langganan',
        'buy' => 'Beli',
        'owned' => 'Dimiliki',
    ],
    'help' => [
        'title' => 'Bantuan',
        'subtitle' => 'Pertanyaan Yang Sering Ditanyakan',
    ],
    'contact' => [
        'title' => 'Kontak Kami',
    ],
]
?>