...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        return Response::json([
            'favorite' => $favorite,
            'fetch' => $fetch,
            'user_avatar' => asset('/storage/' . '/' . $fetch->avatar->medium),
        ]);
    }
    ...
    {
        $path = base_path('../assets/' . config('chatify.attachments.folder') . '/' . $fileName);
        if (file_exists($path)) {
        ...
        if ($request->hasFile('file')) {
            ...
            if ($file->getSize() < 150000000) {
                if (in_array($file->getClientOriginalExtension(), $allowed)) {
                    ...
                    Storage::putFileAs(config('chatify.attachments.folder'), $file, $attachment);
                } else {
    ...
    public function sharedPhotos(Request $request)
    {
        ...
        for ($i = 0; $i < count($shared); $i++) {
            $sharedPhotos .= view('Chatify::layouts.listItem', [
                'get' => 'sharedPhoto',
                'image' => asset('storage/chats/attachments' . $shared[$i]),
            ])->render();
        }
        // send the response
        return Response::json([
